...
"""
CONFIG = {
    "model_path": "/app/oven_machine_b_model86/weights/best.pt",
    "input_dirs": {
    ...
    "no_box_dirs": {
       ...
       "roi_polygons": {
        ...
        "machine-3": [(206, 21), (292, 21), (292, 119), (206, 119)],
...
def gen_new_filename(machine_name, predict_time, detection_result, ext=".jpg"):
    ...
    # เปลี่ยน ":" กับ space เพื่อให้ชื่อไฟล์ปลอดภัย
    predict_time_nfn = predict_time.replace(":", "-").replace(" ", "_")
    detection_result_nfn = detection_result.replace(" ", "_").replace(",", "-")
    ...
    for img_file in image_files:
        ...
        for machine_name, data in machine_data.items():
            ...
            if label_set:
                ...
                if sub_signal != 0 and main_signal != 0:
                ...
                # กรณี no detection
                predict_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                new_filename = gen_new_filename(machine_name, predict_time, "no detection")
                result_dict = {
                    "predict_time": predict_time,
        ...
        if CONFIG["show_frame_predict"]:
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                print("User pressed q, exiting prediction.")
                return
